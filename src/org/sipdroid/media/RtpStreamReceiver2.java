...
public class RtpStreamReceiver2 extends Thread {
	 ...
	 */
	public RtpStreamReceiver2(DatagramSocket socket, Codecs.Map payload_type, CallRecorder rec) {
		init(socket);
		p_type = payload_type;
		call_recorder = rec;
	}
	...
	private void init(DatagramSocket socket) {
		if (socket != null)
			rtp_socket = new RtpSocket2(socket);
	}
	...
	public static void setMode(int mode) {
		...
		if (Integer.parseInt(Build.VERSION.SDK) >= 5) {
			...
			if (samsung) RtpStreamSender2.changed = true;
		} else
			am.setMode(mode);
	}
	...
	void lock(boolean lock) {
		try {
			if (lock) {
				boolean lockNew = (keepon && Receiver.on_wlan) || RtpStreamSender2.delay != 0;
	...
	private static void println(String str) {
		if (!Sipdroid.release) System.out.println("RtpStreamReceiver2: " + str);
	}
