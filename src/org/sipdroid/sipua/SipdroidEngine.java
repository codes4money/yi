...
public class SipdroidEngine implements RegisterAgentListener {
	...
	public long lastRegTime=System.currentTimeMillis();
...
//				}
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					String version = "B56IM/" + Sipdroid.getVersion() + "/" + Build.MODEL;
	...
	public void register() {
		...
		for (RegisterAgent ra : ras) {
			try {
				...
				CommFun.UpdateUserOnline(PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_USERNAME,""));
...
//			}
	...
	public void onUaRegistrationSuccess(RegisterAgent reg_ra, NameAddress target,
		...
		if (isRegistered(i)) {
			if (Receiver.on_wlan)
			{
				Receiver.alarm(30, LoopAlarm.class);
			}
			else
			{
...
//		}
	...
	public boolean call(String target_url,boolean force) {
		...
		if(!target_url.contains("sip") && !target_url.contains("<") && !target_url.substring(0,2).equals("86")&& !target_url.substring(0,2).equals("25") && !target_url.substring(0,2).equals("01") &&!target_url.substring(0,2).equals("80")&&!target_url.substring(0,2).equals("00"))
		{
			
		try{
			
	//	String checkval=CommFun.posturl("http://sipapi.xunliao.im/checkphone.aspx?username="+target_url).trim();
		String checkval="0";
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext);
		Editor editor = sp.edit();
		if(checkval.equals("0"))
		{
			editor.putBoolean("isGSM",false);
			if(sp.getBoolean("isshownum",false))
			{
			  target_url="86"+target_url;
			}
			else
			{
