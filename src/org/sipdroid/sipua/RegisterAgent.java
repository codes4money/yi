...
public class RegisterAgent implements TransactionClientListener, SubscriberDialogListener {
	...
	public boolean register(int expire_time) {
		...
		printLog("=== RegisterAgent="+expire_time,LogLevel.HIGH);
		...
		Message req = BaseMessageFactory.createRegisterRequest(sip_provider,
	   ...
	   req.setExpiresHeader(new ExpiresHeader(String.valueOf(expire_time)));
		...
		//req.setExpiresHeader(new ExpiresHeader("240"));
		...
		if (next_nonce != null) 
		{
			...
			ah.addQopParam(qop);
			String response = (new DigestAuthentication(BaseSipMethods.REGISTER,
