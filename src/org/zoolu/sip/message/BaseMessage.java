...
//		String sent_by = top_via.getSentBy();
	...
	public MultipleHeader getContacts() {
		Vector<Header> v = getHeaders(BaseSipHeaders.Contact);
		if (v.size() > 0)
	...
	public MultipleHeader getVias() {
		Vector<Header> v = getHeaders(BaseSipHeaders.Via);
		if (v.size() > 0)
			return new MultipleHeader(v);
		else
			return null;
	...
	public MultipleHeader getRoutes() {
		Vector<Header> v = getHeaders(BaseSipHeaders.Route);
		if (v.size() > 0)
			return new MultipleHeader(v);
		else
			return null;
	...
	public MultipleHeader getRecordRoutes() {
		Vector<Header> v = getHeaders(BaseSipHeaders.Record_Route);
		if (v.size() > 0)
			return new MultipleHeader(v);
		else
			return null;
