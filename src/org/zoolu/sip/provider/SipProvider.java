...
public class SipProvider implements Configurable, TransportListener,
	...
	private ConnectionIdentifier sendMessage(Message msg, String proto,
		...
		if (transport_udp && proto.equals(PROTO_UDP)) { // UDP
			...
			ConnectedTransport conn = connections
					.get(conn_id);
			if (conn != null) {
		...
		try { // logs
			...
			if (listeners.containsKey(key)) {
				printLog("message passed to transaction: " + key,
						LogLevel.MEDIUM);
				SipProviderListener sip_listener = listeners.get(key);
			...
			if (listeners.containsKey(key)) {
				...
				listeners.get(key).onReceivedMessage(
						this, msg);
				return;
			}
			...
			if (listeners.containsKey(ANY)) {
				...
				listeners.get(ANY).onReceivedMessage(
						this, msg);
				return;
			}
	...
	private void addConnection(ConnectedTransport conn) {
		...
		if (connections.containsKey(conn_id)) { // remove the previous
			...
			printLog("connection " + conn_id + " will be replaced",
					LogLevel.HIGH);
			ConnectedTransport old_conn = connections
					.get(conn_id);
			old_conn.halt();
