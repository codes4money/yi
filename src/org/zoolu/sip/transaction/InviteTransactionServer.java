...
public class InviteTransactionServer extends TransactionServer {
	...
	public InviteTransactionServer(SipProvider sip_provider, Message invite,
		...
		if (auto_trying) {
			Message trying100 = BaseMessageFactory.createResponse(request, 100,
	...
	public void listen() {
		...
		if (statusIs(STATE_IDLE)) {
			...
			sip_provider.addSipProviderListener(new TransactionIdentifier(
					BaseSipMethods.INVITE), this);
			sip_provider.addSipProviderListener(new TransactionIdentifier(
			        BaseSipMethods.OPTIONS), this);
	...
	public void onReceivedMessage(SipProvider provider, Message msg) {
		if (msg.isRequest()) {
				...
				if (statusIs(STATE_WAITING)) {
					...
					sip_provider
							.removeSipProviderListener(new TransactionIdentifier(
									BaseSipMethods.INVITE));
					changeStatus(STATE_TRYING);
					// automatically send "100 Tryng" response and go to
					// STATE_PROCEEDING
					if (auto_trying) {
						Message trying100 = BaseMessageFactory.createResponse(
			...
			if (req_method.equals(BaseSipMethods.OPTIONS)) {
				Message ok200 = BaseMessageFactory.createResponse(msg, 200, SipResponses.reasonOf(200), null);
