...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	public void invite(String callee, String caller, String contact,
		...
		Message invite = BaseMessageFactory.createInviteRequest(sip_provider,
	...
	public void reInvite(String contact, String session_descriptor) {
		...
		if (!statusIs(D_CALL))
			return;
		// else
		Message invite = BaseMessageFactory.createInviteRequest(this,
				session_descriptor);
		if (contact != null) {
	...
	public void ackWithAnswer(String contact, String session_descriptor) {
		...
		Message ack = BaseMessageFactory.create2xxAckRequest(this,
				session_descriptor);
		ackWithAnswer(ack);
	}
	...
	public void respond(int code, String reason, String contact, String sdp) {
		...
		if (statusIs(D_INVITED) || statusIs(D_ReINVITED)) {
			...
			Message resp = BaseMessageFactory.createResponse(invite_req, code,
	...
	public void cancel() {
		...
		if (statusIs(D_INVITING) || statusIs(D_ReINVITING)) {
			Message cancel = BaseMessageFactory.createCancelRequest(invite_req,this); // modified
			cancel(cancel);
	...
	public void onReceivedMessage(SipProvider sip_provider, Message msg) {
		...
		if (msg.isRequest() && msg.isInvite()) {
			...
			Log.v("ÄÚÈÝÖ÷Ìå====1", invite_req.getBody());
		...
		if (msg.isRequest() && msg.isBye()) {
			...
			Message resp = BaseMessageFactory.createResponse(msg, 200, SipResponses
					.reasonOf(200), null);
			respond(resp);
		...
		if (msg.isRequest() && msg.isCancel()) {
			...
			TransactionServer ts = new TransactionServer(sip_provider, msg,
					null);
			// ts.listen();
			ts.respondWith(BaseMessageFactory.createResponse(msg, 200, SipResponses
					.reasonOf(200), null));
			// automatically sends a 487 Cancelled
			Message resp = BaseMessageFactory.createResponse(invite_req, 487,
		...
		if (msg.isRequest()) {
			TransactionServer ts = new TransactionServer(sip_provider, msg,
					null);
			// ts.listen();
			ts.respondWith(BaseMessageFactory.createResponse(msg, 405, SipResponses
					.reasonOf(405), null));
		}
	}
	...
	public void onTransFailureResponse(TransactionClient tc, Message msg) {
		...
		} else if (tc.getTransactionMethod().equals(BaseSipMethods.BYE)) {
	...
	public void onTransSuccessResponse(TransactionClient tc, Message msg) {
		...
		if (tc.getTransactionMethod().equals(BaseSipMethods.INVITE)) {
			...
			if (!verifyThat(code >= 200 && code < 300
					&& msg.getTransactionMethod().equals(BaseSipMethods.INVITE),
