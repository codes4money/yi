...
public class SessionListUtil {
	...
	public static SessionList toDbMessage2(Context context, MessageInfo info, UserInfoVo userInfo,String roomName){
		FinalDb finalDb = FinalFactory.createFinalDb(context, userInfo);
		
		List<SessionList> sessions = finalDb.findAllByWhere(SessionList.class, "fuid = '" + roomName+"@conference" + "'");
		if(sessions != null && !sessions.isEmpty()){
			return sessions.get(0);
		}
