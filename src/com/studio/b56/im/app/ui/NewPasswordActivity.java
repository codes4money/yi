...
public class NewPasswordActivity extends BaseRegisterActivity {
	...
	private void submitNewpassword() {
		...
		if (TextUtils.isEmpty(password)) {
			Toast.makeText(getBaseContext(), "新密码不能为空", Toast.LENGTH_SHORT)
					.show();
			return;
		}

		if (!RegisterUtil.validatePassword(password)) {
			Toast.makeText(getBaseContext(), "密码长度为 6 ~ 12 位",
					Toast.LENGTH_SHORT).show();
			return;
		}

		if (!RegisterUtil.validate(password, repassword)) {
			Toast.makeText(getBaseContext(), "两次密码不相同", Toast.LENGTH_SHORT)
					.show();
			return;
		}
	...
	void submitSuccess() {
		...
		Intent intent = new Intent(NewPasswordActivity.this,
				LoginActivity.class);
