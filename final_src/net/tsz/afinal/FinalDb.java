...
public class FinalDb {
	 ...
	 * @param orderBy 排序的字段
	 */
	public <T> List<T> findAllDESC(Class<T> clazz,String orderBy){
		checkTableExist(clazz);
	 ...
	 * @param orderBy 排序的字段
	 */
	public <T> List<T> findAllASC(Class<T> clazz,String orderBy){
		checkTableExist(clazz);
...
//			return true;
	 ...
	 * 修改时间:<br />
	 */
	public void saveList(List<?> entitys) {
		if (entitys == null || entitys.isEmpty())
			return;
		checkTableExist(entitys.get(0).getClass());
		try {
			db.beginTransaction();
			SqlInfo sqlInfo = SqlBuilder.buildInsertSql(entitys.get(0));
			android.database.sqlite.SQLiteStatement stmt = db.compileStatement(sqlInfo.getSql());
			for (int i = 0; i < entitys.size(); i++) {
				Object[] data = SqlBuilder.buildInsertSql(entitys.get(i)).getBindArgsAsArray();
				stmt.clearBindings();
				for (int j = 0; j < data.length; j++) {
					stmt.bindString(j+1, data[j].toString());
				}
				stmt.executeInsert();
			}
